...
type_gaming = st.selectbox('¿Es Gaming?', ['No', 'Si'])
type_notebook = st.selectbox('¿Es Notebook?', ['No', 'Si'])

# Convertir entradas a formato numèrico
type_gaming = 1 if type_gaming == 'Sì' else 0
type_notebook = 1 if type_notebook == 'Sì' else 0

# Botón para realizar predicción
if st.button('Predecir Precio'):
    # Crear DataFrame con las entradas
    input_data = pd.DataFrame([[ram, screen_width, screen_height, ghz, type_gaming, type_notebook]],
                    columns=['Ram', 'screen_width', 'screen_height', 'GHz', 'TypeName_Gaming', 'TypeName_Notebook'])
